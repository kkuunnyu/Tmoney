...
function loadPage(pageOption) {
  ...
  axios.get(`${url}/common-code`, { params: requestParams })
    .then((res) => {
      if (res.data.success == true) {
        //console.log(`${url}/common-code`, { params: requestParams });
...
function initTable(noContent) {
  ...
  ccList.innerHTML = `
    <tr>
        ...
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">공통코드 영문명</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">공통코드 한글명</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">공통코드값</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">코드값명</td>
    </tr>
    `
  if (noContent) {
    ccList.innerHTML += `
...
async function renderContent(content) {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    row.innerHTML = `
                <td width="50px">${displayNumber}</td>
                <td width="300px">${value.CMN_CD_ENG_NM}</td>
                <td width="300px">${value.CMN_CD_NM}</td>
                <td width="200px">${value.CMN_CD_VAL}</td>
                <td width="150px">${value.CD_VAL_NM}</td>
                `;
...
document.querySelector("#resetBtn").addEventListener("click", function () {
  ...
  window.location.href = "info-common-code?page=0&size=10&sort=0"
});
