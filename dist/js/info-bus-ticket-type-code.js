...
function loadPage(pageOption) {
  ...
  axios.get(`${url}/ticket-kind`, { params: requestParams })
    .then((res) => {
      if (res.data.success == true) {
        //console.log(`${url}/ticket-kind`, { params: requestParams });
...
function initTable(noContent) {
  ...
  btcList.innerHTML = `
    <tr>
        ...
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">버스티켓종류코드</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">버스티켓종류명</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">버스티켓종류단축명</td>
    </tr>
    `
  if (noContent) {
    btcList.innerHTML += `
...
async function renderContent(content) {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    row.innerHTML = `
                ...
                <td>${value.BUS_TCK_KND_CD}</td>
                <td>${value.BUS_TCK_KND_NM}</td>
                <td>${value.BUS_TCK_KND_SHCT_NM}</td>
                `;
...
document.querySelector("#resetBtn").addEventListener("click", function () {
  ...
  window.location.href = "info-bus-ticket-type-code?page=0&size=10&sort=0"
});
