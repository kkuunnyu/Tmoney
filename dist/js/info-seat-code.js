...
function loadPage(pageOption) {
  ...
  axios.get(`${url}/seat-architecture`, { params: requestParams })
    .then((res) => {
      if (res.data.success == true) {
        //console.log(`${url}/seat-architecture`, { params: requestParams });
...
function initTable(noContent) {
  ...
  scList.innerHTML = `
    <tr>
        ...
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">좌석배치도코드</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">좌석수</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">좌석배치유형코드</td>
    </tr>
    `
  if (noContent) {
    scList.innerHTML += `
...
async function renderContent(content) {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    row.innerHTML = `
                ...
                <td>${value.SATS_ARCT_CD}</td>
                <td>${value.SATS_NUM}</td>
                <td>${value.SATS_ARCT_TYP_CD}</td>
                `;
...
document.querySelector("#resetBtn").addEventListener("click", function () {
  ...
  window.location.href = "info-seat-code?page=0&size=10&sort=0"
});
