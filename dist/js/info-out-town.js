...
function loadPage(pageOption) {
  ...
  axios.get(`${url}/city-message`, { params: requestParams })
    .then((res) => {
      if (res.data.success == true) {
        //console.log(`${url}/city-message`, { params: requestParams });
...
function initTable(noContent) {
  ...
  otList.innerHTML = `
    <tr>
        ...
        <td bgcolor="#f0f0f0" width="300px" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">시외버스 메시지 코드</td>
        <td bgcolor="#f0f0f0" style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: 600;">시외버스 메시지명</td>
    </tr>
    `
  if (noContent) {
    otList.innerHTML += `
        <tr>
            <td colspan=3> 검색에 해당하는 내용이 없습니다. </td>
        </tr>
        `
...
async function renderContent(content) {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    const formattedMSG = value.CTY_BUS_MSG_NM.replace(/\\n/g, "<br>");
    row.innerHTML = `
                <td class="table-no">${displayNumber}</td>
                <td>${value.CTY_BUS_MSG_CD}</td>
                <td>${formattedMSG}</td>
                `;
